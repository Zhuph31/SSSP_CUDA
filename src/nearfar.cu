...
index_type* new_near_len, index_type* new_far_pile, index_type* new_far_len, index_type* scratch) {
    ...
    if (index < last_near_len) {
        ...
        for (int j = g.row_start[s_idx]; j < g.row_start[s_idx + 1]; j++) {
            ...
            if (new_w < nw) {
                ...
                index_type q_idx;
                if (new_w < delta){
                    //near pile, this value has been updated. Flag this value need to be thrown out in far pile 
...
index_type* new_near_len, index_type* new_far_pile, index_type* new_far_len, index_type* scratch) {
    ...
    if (index < last_far_len) {
        ...
        //remove duplicates in the far pile, and remove flagged invalid edge from near pile update
...
void nearfar(CSRGraph& g, edge_data_type* dists) {
    ...
    *near_len = 1, *far_len = 1;
    ...
    //calculate delta for graph, calculated delta seems to be too large, divide by 10 gets good results on rmat22
    ...
    while (*far_len > 0 || *near_len > 0) {
        ...
        *near_len = 0;
        // keep 
        nf_iter<<<(old_near_len + 512 - 1) / 512,512>>>(d_g, d_d, delta, near1, old_near_len, near2, near_len, far2, far_len, qscratch);
        ...
        if (*near_len == 0){
            ...
            while (*near_len == 0){
                ...
                *far_len = 0; 
                *near_len = 0; 
                far_split<<<(old_far_len + 512 - 1) / 512,512>>>(d_d, delta, far2, old_far_len, near1, near_len, far1, far_len, qscratch);
                cudaDeviceSynchronize();
            }
            index_type* tmp = far1;
            far1 = far2;
